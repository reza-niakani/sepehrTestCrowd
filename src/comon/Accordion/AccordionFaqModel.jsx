...
const AccordionFaqModel = ({ question, answer, HeaderColor = false, bgColor = false }) => {
    ...
    <div
      ...
      <div
        ...
        <h3
          className={`lg:text-base font-bold w-full text-sm ${
            HeaderColor ? isOpen && HeaderColor : 'text-gray-700'
          } text-start `}>
          {question}
        </h3>
        <InlineSVG
      ...
      <div
        className={`overflow-hidden transition-all duration-500 ease-in-out w-[95%]  ${
          isOpen ? (bgColor ? 'max-h-[300px] overflow-y-auto' : 'max-h-screen border-b') : 'max-h-0'
        }`}>
        <div
          style={{ lineHeight: '25px' }}
          className={`p-4  ${
            bgColor ? '' : 'rounded-b-lg'
          } lg:text-base text-xs text-gray-700   text-justify`}>
          {answer}
        </div>
      </div>
    </div>
