...
import reportpic from 'asset/Pictures/Images/reportPic.svg';
...
function ReportsCard({ data }) {
  const findStatusTitle = () => {
    switch (data?.status) {
      ...
      case 2:
        return (
          <div className="lg:min-w-[90px] min-w-[80px] max-w-[110px] w-auto h-[28px] rounded-md bg-[#E4FFED] text-green-1000 flex text-center justify-center items-center lg:text-sm lg:font-semibold text-xs  ">
            تایید شده{' '}
          </div>
        );
      case 3:
  ...
  return (
    <div className="w-full flex lg:h-[106px] h-[165px] rounded-lg drop-shadow-lg  bg-white border border-green-1000  lg:justify-center justify-center gap-y-5 items-center lg:flex-row flex-col  ">
      <div className="lg:w-[50%] w-full flex justify-start items-center gap-x-2 ">
        {' '}
        {/*  deafult pic */}
        <InlineSVG src={reportpic} className="w-[80px] h-[80px]" />
        {/*  file data  */}
        <div className=" lg:max-w-[70%] lg:min-w-[40%] lg:w-auto w-full text-sm  text-gray-700 flex flex-col justify-start gap-y-2 items-start   ">
