...
const PaginationComponet = ({ total, currentPage, setCurrentPage, pageSize }) => {
  ...
  return (
    <>
      {' '}
      {total > pageSize && (
        <div className="w-full flex gap-x-2 items-center justify-center rounded-lg h-[35px]  ">
          {pageRange.map((page, index) => (
            <button
              key={index}
              onClick={() => page !== '...' && setCurrentPage(page)}
              className={` h-full border-0 focus:outline-none focus:border-0  w-[20px] text-base  font-medium   focus:right-0 ${
                page === currentPage ? '  text-green-1000  ' : ' text-gray-700  '
              }`}
              disabled={page === '...'}>
              {page}
            </button>
