...
const CustomDropDown = ({ options, selectedItem, setSelectedItem, label, width }) => {
  ...
  return (
    <div ref={dropdownRef} className={`relative inline-block ${width ? width : 'w-full'}`}>
      <div className="w-full flex justify-center gap-0 flex-nowrap items-center h-[42px] border border-[#E0E0E0] rounded-lg">
        {label && (
          <>
            {' '}
        ...
        <div
          ...
          {options?.map((item) => (
            <span
              ...
              className={`px-4 py-2 text-xs w-full h-[42px] rounded-lg cursor-pointer hover:bg-green-100 ${
                selectedItem?.key === item.key ? 'bg-green-100' : ''
              }`}>
              {item.name}
            </span>
          ))}
