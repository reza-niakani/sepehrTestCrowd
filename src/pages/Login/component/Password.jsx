...
function Password({ step, setStep, nationalId, otp, captchaValue, captcha }) {
  ...
  return (
    <div className=" w-full flex flex-col h-full justify-center  items-center gap-y-10  ">
      <span className=" text-gray-700 font-black lg:text-lg text-lg text-start w-full ">
      ...
      <form
        ...
        <div className="w-full flex flex-col justify-start items-end h-auto gap-y-2">
          <div className="w-full justify-start flex items-center ">
            ...
            <label
              htmlFor="password"
              className=" text-gray-700 w-fit text-nowrap  text-start  items-center lg:text-sm text-xs ">
              {step == 'password' ? 'رمز عبور ' : 'ایجاد رمز عبور'}{' '}
            </label>
            {step == 'password' && (
              <span
                className="w-full flex justify-end   text-end  lg:text-sm text-xs text-green-1000 items-center  cursor-pointer "
          ...
          <input
            ...
            className={`w-full   ${
              step == 'password'
                ? 'text-gray-500 text-center border border-gray-70 '
                : password && passwordConfirm
                  ? password == passwordConfirm
                    ? 'text-gray-500 border border-gray-70'
                    : 'border-red-600 focus:border-red-600  text-red-600 '
                  : 'text-gray-500 border border-gray-70 '
            } rounded-lg text-center  pr-2 text-sm  focus:border-gray-70  focus:ring-0 focus:outline-none placeholder:text-gray-500  lg:h-[42px] h-[40px]  `}
          />
          <InlineSVG
        ...
        {(step == 'newPassword' || step == 'registerPassword') && (
          <div className="w-full flex flex-col justify-start items-end h-auto gap-y-1">
            <label
              ...
              className="text-sm text-gray-700 w-full font-medium text-start items-center lg:pr-3">
              تکرار رمز عبور{' '}
            </label>
            <input
        ...
        {passwordConfirm && password && password !== passwordConfirm && (
          <span className="w-full flex lg:justify-start justify-center  text-center lg:text-start text-xs text-red-600 items-center lg:pr-2">
        ...
        {/* password roles  */}
        {step !== 'password' && (
          <div className="w-full flex flex-col items-center  justify-start gap-y-2 text-gray-700  text-xs pt-2  ">
        ...
        <div className="w-full flex  justify-between  items-end h-[120px] gap-y-2 pt-2">
          ...
          <button
            className="rounded-large text-sm lg:h-[50px] h-[43px] text-gray-700 w-[49%] shadow-dropShadow2  "
