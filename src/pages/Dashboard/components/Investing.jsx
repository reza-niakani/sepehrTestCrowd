    ...
    <div className="w-full flex flex-col justify-start items-center gap-y-4 pb-24 lg:pb-0">
      ...
      <div className="lg:w-[95%] w-full flex items-center justify-between">
        <div className="w-auto flex-nowrap items-center flex justify-start gap-x-2">
          ...
          <span className=" text-base text-gray-700 font-medium ">آخرین طرح ها </span>
        </div>
        <Link
          to="/investing"
      ...
      <div className="w-full flex flex-wrap gap-10 items-center justify-center ">
        {isloading ? (
              ...
              <Swiper
                ...
                {plans?.map((item, index) => (
                  <SwiperSlide
                    ...
                    style={{ display: 'flex' }}>
                    {' '}
                    <InvestinDashBoardCard key={index} data={item} />{' '}
                  </SwiperSlide>
                ))}{' '}
              </Swiper>
          ...
          plans?.map((plan, index) => <InvestinDashBoardCard key={index} data={plan} />)
        ) : (
          <span className="lg:w-[50%] w-[90%] text-center text-sm font-bold text-green-1000 bg-[#01B69B08]   border border-green-1000 rounded-large h-[42px]  flex items-center justify-center">
            طرحی برای سرمایه گذاری یافت نشد!
          </span>
        )}
      </div>
