...
function Changepassword({ userName }) {
  ...
  return (
    <div className="w-full min-h-[400px] justify-center items-center h-auto flex flex-col gap-y-5 ">
      ...
      {otpStatus && (
        <>
          <div className=" flex lg:w-[40%] w-[90%]  flex-col gap-y-2 justify-start items-end ">
            <label htmlFor="otp" className="text-xs text-gray-700 font-medium  text-start w-full">
              {' '}
              کد ارسالی{' '}
            </label>
            <input
              ...
              maxLength={5}
              inputMode="numeric"
              className="w-full text-sm text-center text-gray-700 pr-2 h-[42px] rounded-lg bg-transparent focus:outline-none border border-[#E0E0E0] placeholder:text-xs placeholder:text-[#8b8b8b]  "
              value={otp}
          ...
          <div className="lg:w-[40%] w-[90%] justify-start items-center flex flex-col gap-y-5">
            ...
            <div className="w-full flex flex-col items-center justify-start gap-y-2">
              <label htmlFor="password" className="text-sm text-gray-700 font-medium w-full">
                رمز عبور
              </label>
            ...
            <div className="w-full flex flex-col items-center justify-start gap-y-1">
              <label htmlFor="confirmPassword" className="text-sm text-gray-700 font-medium w-full">
                تکرار رمز عبور
              </label>
              <div className="relative w-full">
                <input
                  ...
                  className={`w-full text-sm text-center pr-2 h-[42px] rounded-lg  focus:ring-0 placeholder:text-xs placeholder:text-[#8b8b8b]  focus:outline-none ${
                    newPassword && confirmedPassword
                      ? newPassword === confirmedPassword
                        ? 'text-gray-500 border border-[#E0E0E0]'
                        : 'text-red-600 border border-red-600 focus:border-red-600'
                      : 'text-gray-500 border border-[#E0E0E0]'
                  }`}
            ...
            {(newPassword && confirmedPassword && newPassword) !== confirmedPassword && (
              <span className="w-full text-center  text-xs text-red-600">
                رمز عبور و تکرار آن متفات است
              </span>
            )}
            {/* Password Rules */}
            <div className="w-full flex flex-col items-center justify-start  gap-y-2 text-gray-700 text-xs ">
      ...
      {otpStatus ? (
        response !== true && response !== false ? (
          <button
            ...
            disabled={!disable}
            className={` lg:w-[40%] w-[90%] flex text-center items-center justify-center text-sm text-green-1000 border border-green-1100 drop-shadow-lg bg-[#F3FAFB]  h-12 rounded-md ${
          ...
          <span
            className={`w-full text-center flex justify-center text-sm ${
              response ? 'text-green-600' : 'text-red-600'
            }  items-center  py-2 `}>
        ...
        <div className=" w-full flex justify-center items-center gap-y-5 flex-col ">
          <span className=" text-center   text-green-1000 font-medium   lg:text-base text-sm ">
            {' '}
      ...
      {otpStatus == false && (
        <span className="w-full text-center flex justify-center text-sm text-red-600 items-center  py-2 ">
